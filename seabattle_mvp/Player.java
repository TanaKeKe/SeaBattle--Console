...
public final class Player extends Board {
    ...
    public void setCoordinates() {
        while (true) {
            ...
            if(this.x1 > 10 || this.y1 > 10 || this.x2 > 10 || this.y2 > 10||
               this.x1 < 1 || this.y1 <1 || this.x2 <1 || this.y2 <1){ // trường hợp đặt thuyền ngoài bảng
                System.out.println("Boats placed outside the front! Enter 2 coordinates (x,y): ");
            }
            else break;
        }
    }
    ...
    public void setShip(int number) { // đặt thuyền + lưu độ dài của thuyền đã đặt
        if (this.x1 == this.x2) {
            if (Math.abs(this.y1 - this.y2) != number) { // kiểm tra độ dài hợp lệ hay không
                System.out.println(RED_BACKGROUND +"INVALID! Wrong ship size!" + ANSI_RESET);
            } else {
                if (this.y1 > this.y2) {
                    int tmp = this.y1;
                    this.y1 = this.y2;
                ...
                if (check_pos == 1) {
                    System.out.println(RED_BACKGROUND + "INVALID! There is already a ship located at that location!" + ANSI_RESET);
                } else { // tại tọa độ đó chưa có tàu đặt thì đặt tàu
                    System.out.println(RED_BACKGROUND + "Booked the boat successfully!" + ANSI_RESET);
                    for (int pos = this.y1 - 1; pos <= this.y2 - 1; ++pos) {
    ...
    public Player() {
        ...
        while (true) {
            ...
            if (number == 1 && this.check_ship[number] <= 2) { // kiểm tra bằng số mấy và số đấy đã được điền tàu chưa
                ...
                System.out.println(RED_BACKGROUND + "INVALID!" + ANSI_RESET);
            }
            displayMyboard(this.Name);
            if (this.cnt_ship == 5) {
                break;
