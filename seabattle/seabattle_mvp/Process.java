...
public class Process {
    ...
    private int check_shoot_hit_enemy = 0;
    ...
    public void displayRule() {
        ...
        System.out.println("not yet fired. If you hit an enemy boat, you will be fired again. GoodLuck");
        System.out.println("-------------------------------------------------------------------------");
        System.out.println("");
    }
    ...
    public void displayMenu(String NamePlayer1, String NamePlayer2) {
        ...
        for (int row = 0; row < 10; ++row) {
            ...
            for (int column = 0; column < 10; ++column) {
                if (this.player1.MyBoard[row][column] != 'X') {
                    System.out.print(this.player1.MyBoard[row][column] + " |");
                } else {
                    System.out.print(Color.RED_BACKGROUND + this.player1.MyBoard[row][column] + Color.ANSI_RESET + Color.RED_BACKGROUND + " " + Color.ANSI_RESET);
                    if (this.player1.MyBoard[row][column + 1] == 'X') {
                        System.out.print(Color.RED_BACKGROUND + " " + Color.ANSI_RESET);
                    } else {
                        System.out.print("|");
                    }
                }
            ...
            for (int column = 0; column < 10; ++column) {
                if (this.player2.MyBoard[row][column] != 'X') {
                    System.out.print(this.player2.MyBoard[row][column] + " |");
                } else {
                    System.out.print(Color.RED_BACKGROUND + this.player2.MyBoard[row][column] + Color.ANSI_RESET + Color.RED_BACKGROUND + " " + Color.ANSI_RESET);
                    if (this.player2.MyBoard[row][column + 1] == 'X') {
                        System.out.print(Color.RED_BACKGROUND + " " + Color.ANSI_RESET);
                    } else {
                        System.out.print("|");
                    }
        ...
        System.out.println(Color.GREEN_BACKGROUND+"-------------------------------------------------------------------------"+Color.ANSI_RESET);
    }
    ...
    public void shootEnemy(Player currentPlayer, Player remainPlayer) {
        ...
        System.out.println(Color.RED_BACKGROUND + "Please enter the coordinates (x,y) you want to shoot: " + Color.ANSI_RESET);
        System.out.print("x = ");
        ...
        if (x > 10 || y > 10 || x < 1 || y < 1) {
            System.out.println(Color.RED_BACKGROUND + "Missed shot!" + Color.ANSI_RESET);
        } else {
            if (currentPlayer.EnemyBoard[x - 1][y - 1] != ' ') {
                System.out.println(Color.RED_BACKGROUND+"This box has been shot, you lose your turn!"+Color.ANSI_RESET);
                this.check_shoot_hit_enemy = 0;
            } else {
                if (remainPlayer.MyBoard[x - 1][y - 1] == 'X') { // nếu tại đó có thuyền của địch
                    this.check_shoot_hit_enemy = 1;
                    System.out.println(Color.RED_BACKGROUND+"You have hit the enemy ship!"+Color.ANSI_RESET);
                    currentPlayer.EnemyBoard[x - 1][y - 1] = 'X';
                    currentPlayer.setBullet();
                } else {
                    System.out.println(Color.RED_BACKGROUND+"You didn't hit the enemy ship!"+Color.ANSI_RESET);
                    this.check_shoot_hit_enemy = 0;
    ...
    public void inforPlayer(Player currentPlayer, Player remainPlayer) {
        ...
        for (int pos = 0; pos < remainPlayer.Coordinates_X.size(); ++pos) {
            ...
            if (x1 == x2) { // nếu cùng tọa độ hành => thuyền nằm ngang
                ...
                if (check == 0) {
                    ++number; // nếu tại đó tàu bị bắn thủng
                }
        ...
        for (int pos = 0; pos < currentPlayer.Coordinates_X.size(); ++pos) {
            ...
            if (x1 == x2) { // nếu cùng tọa độ hành => thuyền nằm ngang
                ...
                if (check == 0) {
                    ++number_1; // nếu tại đó tàu bị bắn thủng
                }
            } else {
                int check = 0; // biến kiểm tra
                for (int Pos = x1 - 1; Pos <= x2 - 1; ++Pos) { // kiểm tra đã bắn thủng tàu tại tọa độ đó hay chưa
    ...
    public void play() {
        ...
        while (true) {
            ...
            if (this.check_win == 1) { // kiểm tra kết thúc trò chơi chưa
                // hiển thị bảng của cả 2 bên đã có ô bị đạn bắn
                for (int row = 0; row < 10; ++row) { // set up lại bảng của người chơi hiện tại theo bảng ẩn và bảng có sẵn
                    for (int column = 0; column < 10; ++column) {
                        if (this.player2.EnemyBoard[row][column] == '*') {
                            this.player1.MyBoard[row][column] = '*';
                        }
                    }
                }
                for (int row = 0; row < 10; ++row) { // set up lại bảng của người chơi hiện tại theo bảng ẩn và bảng có sẵn
                    for (int column = 0; column < 10; ++column) {
                        if (this.player1.EnemyBoard[row][column] == '*') {
                            this.player2.MyBoard[row][column] = '*';
                        }
                    }
                }
                displayMenu(this.player1.getName(), this.player2.getName());
                System.out.println("End Game!");
                break;
            }
            if (currentPlayer == this.player1 && this.check_shoot_hit_enemy == 0) { // đổi lượt chơi cho người còn lại nếu người hiện tại không bắn trúng
                currentPlayer = this.player2;
                remainPlayer = this.player1;
            } else if (currentPlayer == this.player2 && this.check_shoot_hit_enemy == 0) {
